...
public:
    ...
    void clear();
    bool getDeleted(const std::string &key);
...
private:
    ...
    std::string deleted[20];
    int d1=0;
    
    Node** newHash;    //2d hashmap to store linked lists
    
    // You will no doubt need to add at least a few more private members
    void arrayCopy(Node** target, Node** const &source, int size);
    
    void initializeArray(Node** &n, int size);
